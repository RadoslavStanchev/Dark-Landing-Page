(function attachEvents() {
    ...
    function addConsultation(event) {
        ...
        let validLecturer = lecturerValidation();
        let filledInputFields = filledInputFieldsValidation();

        if(validUsername && validLecturer && filledInputFields) {
    ...
    function addNewConsultation() {
        ...
        $('.education article:nth-child(3) .box-body ul').append(`<li><span>${lecturerName} - ${monthDay} - ${time}</span>
        <i class="fas fa-chevron-circle-right"></i></li>`);
